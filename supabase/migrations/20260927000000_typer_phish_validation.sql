-- ===================================================================
-- OWL GAMES — 나이트 타이퍼(typer) · 피싱 헌터(phish) 서버 검증
--
-- 배경
--   아울러닝·아울 로직·아울 서바이버즈는 서버가 메타로 원점수를 다시 계산하는데
--   타이퍼·피싱만 클라이언트 점수를 그대로 받고 있었다. 상한(300P)과 아울 에너지가
--   막아 주긴 하지만, 같은 행사에서 게임마다 판정이 다른 건 공정하지 않다.
--
-- 무엇을 하는가
--   1) public.phish_max_score(correct, max_streak) — 피싱에서 만들 수 있는 최대 점수
--   2) submit_game_session 에 typer·phish 메타 검증과 원점수 상한을 추가
--      · typer : 방화벽 = 놓친 수 × 20%p(상한 100), 단어 수 ≤ 스폰 가능 수, 콤보 ≤ 파괴 수
--      · phish : 판별 수 = 정답 + 오답, 경과시간 ≤ 90 − 5 × 오답 + 8
--      · 두 게임 모두 "재계산"이 아니라 **상한 클램프**다 (넘으면 meta.raw_adjusted 기록)
--
-- 주의
--   20260926 버전의 함수 본문을 그대로 두고 분기만 끼워 넣었다.
--   enum 값 비교는 계속 `v_s.game::text = '...'` 형태만 쓴다 (같은 트랜잭션 제약).
-- ===================================================================

-- ===== 1. 피싱 최대 점수 =============================================
-- 정답 1장 = 100 × min(2.0, 1 + 0.1 × 직전 연속 정답 수) (games/phish/PhishGame.tsx)
-- 길이 L 연속의 합 f(L) = 100 × ( m + 0.05·m·(m−1) + 2·(L−m) ),  m = min(L, 11)
-- 정답 C개를 최고 연속 S로 쪼갤 때 최대 = floor(C/S) × f(S) + f(C mod S)
--   (연속이 길수록 장당 점수가 높으니, S 길이 뭉치를 최대한 많이 만드는 쪽이 최대값이다)
create or replace function public.phish_streak_score(p_len int)
returns numeric
language sql
immutable
...
as $$
  select case
           when coalesce(p_len, 0) <= 0 then 0
           else 100 * ( least(p_len, 11)
                      + 0.05 * least(p_len, 11) * (least(p_len, 11) - 1)
                      + 2 * greatest(0, p_len - 11) )
         end;
$$;

create or replace function public.phish_max_score(p_correct int, p_streak int)
returns numeric
language plpgsql
immutable
set search_path = public, pg_temp
as $$
declare
  v_c int := greatest(0, coalesce(p_correct, 0));
  v_s int := greatest(0, coalesce(p_streak, 0));
begin
  if v_c = 0 then
    return 0;
  end if;
  -- 최고 연속이 없거나 정답 수보다 크면 "전부 한 줄"로 본다 (검증 단계가 s ≤ c 를 이미 보장한다)
  if v_s <= 0 or v_s > v_c then
    v_s := v_c;
  end if;
  return (v_c / v_s) * public.phish_streak_score(v_s)
       + public.phish_streak_score(v_c % v_s);
end;
$$;

revoke execute on function public.phish_streak_score(int) from public, anon, authenticated;
revoke execute on function public.phish_max_score(int, int) from public, anon, authenticated;
grant execute on function public.phish_streak_score(int) to authenticated;
grant execute on function public.phish_max_score(int, int) to authenticated;


-- ===== 2. submit_game_session (typer·phish 분기 추가) ================
