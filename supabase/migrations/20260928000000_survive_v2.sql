-- ===================================================================
-- OWL GAMES — 아울 서바이버즈 v2 (OWLSURVIVORS_GDD_v2)
--
-- v1(구역 3개 · 한 판에 다 도는 방식)을 폐기하고 **한 스테이지 = 한 런**으로 바꾼다.
--   · 보스를 잡아야 클리어, 클리어해야 다음 스테이지가 열린다 (profiles.meta.survive_stage)
--   · 스테이지 1~15 + 16부터 무한. 점수 배율 1 + (S-1)×0.06 (상한 ×3.0)
--   · meta 키 구성이 통째로 바뀐다 (§11.3)
--
-- 무엇을 하는가
--   1) game_limits.survive 하한 20초 (§16)
--   2) game_guards 설정 신설 — 거부 기준을 코드가 아니라 설정으로 둔다 (§16)
--   3) profiles.meta.survive_unlock → survive_stage 이관 (+ survive_theme)
--   4) submit_game_session 의 survive 분기를 v2 규칙으로 교체
--
-- 주의
--   20260927 버전 본문을 그대로 두고 survive 분기만 갈아끼웠다.
--   enum 비교는 계속 `v_s.game::text = '...'` 만 쓴다.
-- ===================================================================

-- ===== 1. 세션 시간 제한 (§16) ======================================
update public.app_config
   set value = jsonb_set(value, '{survive}', '{"min_sec":20,"max_sec":200}'::jsonb, true)
 where key = 'game_limits';

-- ===== 1-2. 환산 계수 K (§11.2) =====================================
--   v1 에서 K=30 으로 올려 뒀지만(그때는 한 판에 구역 3개를 다 돌았다),
--   v2 는 한 스테이지만 돌아서 원점수가 작아진다 → 기획서대로 20 으로 되돌린다.
update public.app_config
   set value = jsonb_set(value, '{survive}', to_jsonb(20), true)
 where key = 'game_k';

-- ===== 2. 거부 기준 (§16 game_guards) ===============================
insert into public.app_config (key, value)
values ('game_guards',
        '{"survive":{"max_kills_per_sec":8,"max_level":24,"max_evolutions":3,"min_clear_sec":90}}'::jsonb)
on conflict (key) do update
   set value = jsonb_set(public.app_config.value, '{survive}',
                 '{"max_kills_per_sec":8,"max_level":24,"max_evolutions":3,"min_clear_sec":90}'::jsonb, true);

-- ===== 3. 진행도 이관 ===============================================
--   v1 의 survive_unlock(1~3, "고를 수 있는 단계")을 v2 의 survive_stage("클리어한 최고 단계")로 옮긴다.
--   unlock 3 = 2단계까지 깼다는 뜻이므로 -1 한다.
update public.profiles
   set meta = jsonb_set(
                coalesce(meta, '{}'::jsonb) - 'survive_unlock',
                '{survive_stage}',
                to_jsonb(greatest(0, coalesce((meta ->> 'survive_unlock')::int, 1) - 1)),
                true)
 where meta ? 'survive_unlock';

-- v2 스테이지 정의는 클라이언트(games/survive/data/stages.ts)가 들고 있다.
-- v1 의 survive_stages(구역 배율 테이블)는 더 이상 쓰지 않는다.
delete from public.app_config where key = 'survive_stages';


-- ===== 4. submit_game_session (survive 분기 교체) ===================
